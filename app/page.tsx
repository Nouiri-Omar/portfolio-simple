...
function PageFace({ page, side }: { page: number; side: "left" | "right" }) {
  ...
  if (page === totalPages - 1) return side === "left" ? <BackCoverPage /> : <div className="empty-page" />;
  if (page === 1) return side === "left" ? <PrefaceLeft /> : <PrefaceRight />;
  const project = projects[page - 2];
  return side === "left" ? <ProjectLeft project={project} index={page - 2} /> : <ProjectRight project={project} index={page - 2} />;
}

function PageContent({ page }: { page: number }) {
  if (page === 0) return <CoverPage />;
  if (page === totalPages - 1) return <BackCoverPage />;
  if (page === 1) return <div className="spread"><PrefaceLeft /><PrefaceRight /></div>;
  const project = projects[page - 2];
  return <div className="spread" aria-label={`Project ${page - 1}: ${project.title}`}><ProjectLeft project={project} index={page - 2} /><ProjectRight project={project} index={page - 2} /></div>;
}
